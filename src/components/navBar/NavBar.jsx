...
export const NavBar = () => {
    ...
    const searchBarChangeHandler = (event) => {
        if (event?.target?.value?.length === 0) {
            ...
            setSearchResults(
                [
                    ...data?.filter((meeting) => meeting?.title?.slice(0, event?.target?.value.length).toLowerCase() === event?.target?.value.toLowerCase()),
                    ...data?.filter((meeting) => meeting?.title?.slice(0, event?.target?.value.length).toLowerCase() !== event?.target?.value.toLowerCase())
                        ?.filter((meeting) => meeting?.title.concat(meeting.eventTags.join('')).toLowerCase()?.includes(event?.target?.value?.toLowerCase()))
                ]
            )
        }
    }
